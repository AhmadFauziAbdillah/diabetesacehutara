    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center mb-4">
        ...
        <div class="map-controls">
            <select id="mapColorBy" class="form-select form-select-sm">
                ...
                <option value="cluster" <?php echo $map_default_color === 'cluster' ? 'selected' : ''; ?>>Berdasarkan Cluster</option>
            </select>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getColor(value, max, type) {
        ...
        if (type === 'cluster') {
            // Discrete colors for clusters
            switch(parseInt(value)) {
                case 0: return '#1a9850'; // Low risk (green)
                case 1: return '#fd8d3c'; // Medium risk (orange)
                case 2: return '#bd0026'; // High risk (red)
                default: return '#cccccc'; // Undefined (gray)
            }
        }
    ...
    function updateMap() {
        ...
        regionsData.forEach(function(region) {
            ...
            // Determine circle color and size based on selected metric
            let circleColor, value, maxValue, popupText;
            
            if (colorBy === 'cluster') {
                circleColor = getColor(region.cluster, 0, 'cluster');
                popupText = `<strong>${region.wilayah}</strong><br>
                ...
                value = region.jumlah_kematian;
                maxValue = maxKematian;
                circleColor = getColor(value, maxValue);
                popupText = `<strong>${region.wilayah}</strong><br>
                            <p>Populasi: ${parseInt(region.jumlah_penduduk).toLocaleString()}</p>
                            <p>Penderita: ${parseInt(region.jumlah_penderita).toLocaleString()}</p>
                            <p>Kematian: <strong>${parseInt(value).toLocaleString()}</strong></p>`;
            } else {
                // Default to penderita
                value = region.jumlah_penderita;
                maxValue = maxPenderita;
                circleColor = getColor(value, maxValue);
                popupText = `<strong>${region.wilayah}</strong><br>
                            <p>Populasi: ${parseInt(region.jumlah_penduduk).toLocaleString()}</p>
                            <p>Penderita: <strong>${parseInt(value).toLocaleString()}</strong></p>
                            <p>Kematian: ${parseInt(region.jumlah_kematian).toLocaleString()}</p>`;
            }
    ...
    function updateLegend() {
        ...
        if (colorBy === 'cluster') {
            // Cluster legend
            const items = [
                { color: '#1a9850', label: 'Cluster 0 (Rendah)' },
                { color: '#fd8d3c', label: 'Cluster 1 (Sedang)' },
                { color: '#bd0026', label: 'Cluster 2 (Tinggi)' },
                { color: '#cccccc', label: 'Tidak Terklasifikasi' }
            ];
